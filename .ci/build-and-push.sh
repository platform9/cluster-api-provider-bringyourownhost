...
# build-and-push.sh - CI script for building and publishing the byoh manager Docker image.
#
# Parameters:
# - IMAGE_REGISTRY  Registry to publish the Docker image. By default 'quay.io/platform9/byoh-manager' is used.
# - IMAGE_NAME      Name to use for this image. By default 'byoh-manager' is used.
# - IMAGE_TAG       Tag to use for the image. By default '$BYOH_VERSION-$BUILD_NUMBER' is used.
...
#
# Examples:
# - `USE_SYSTEM_GO=1 IMAGE_REGISTRY=quay.io IMAGE_NAME=platform9/byoh-manager IMAGE_TAG=latest ./build-and-push.sh`: To test the script locally without gimme and push to Docker
...
BYOH_VERSION=${BYOH_VERSION:-0.1}

IMAGE_REGISTRY=${IMAGE_REGISTRY:-"quay.io/platform9"}
IMAGE_NAME=${IMAGE_NAME:-"byoh-manager"}
IMAGE_TAG=${IMAGE_TAG:-${BYOH_VERSION}.${BUILD_NUMBER}}
...
main() {
  ...
  make docker-build IMG="${IMAGE_REGISTRY_NAME_TAG}" GIT_TOKEN=${GITHUB_TOKEN} GIT_USER=${GITHUB_USER}
