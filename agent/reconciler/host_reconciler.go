...
func (r *HostReconciler) reconcileNormal(ctx context.Context, byoHost *infrastructurev1beta1.ByoHost) (ctrl.Result, error) {
	...
	// Throttled heartbeat stamp: bounds the actual write (and hence the
	// resourceVersion bump / watch notification the management controller
	// sees) to at most once per HeartbeatInterval, no matter how often
	// Reconcile itself is invoked (its own RequeueAfter timer, or any
	// incidental watch event).
	now := metav1.Now()
