...
func TestNewCSRName(t *testing.T) {
	first := newCSRName(testHostName)
	second := newCSRName(testHostName)

	assert.True(t, strings.HasPrefix(first, ByohCSRNamePrefix), "the approver filters on this prefix")
	assert.Contains(t, first, testHostName)
	assert.NotEqual(t, first, second, "a denied request must not be reattached to on the next attempt")
	assert.Len(t, first, len(ByohCSRNamePrefix)+len(testHostName)+1+csrNameSuffixLength)
}
...
func TestEnsureClientCertRequest(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// The UID is assigned by the API server, which the fake client
			// does not do, so only the name is worth asserting on here.
			name, _, err := bcsr.ensureClientCertRequest(t.Context(), testHostName)
			require.NoError(t, err)

			if tc.reusedName != "" {
				assert.Equal(t, tc.reusedName, name)
			} else {
				assert.True(t, strings.HasPrefix(name, ByohCSRNamePrefix+testHostName+"-"), "expected a freshly named request, got %q", name)
			}
...
func TestEnsureClientCertRequestRejectsEmptyHostName(t *testing.T) {
	localTestKey(t)
	...
	_, _, err := bcsr.ensureClientCertRequest(t.Context(), "")
...
func TestRequestBYOHClientCertLabelsAndNamesTheRequest(t *testing.T) {
	...
	name, _, err := bcsr.RequestBYOHClientCert(t.Context(), testHostName)
	require.NoError(t, err)

	created, err := client.CertificatesV1().CertificateSigningRequests().Get(t.Context(), name, metav1.GetOptions{})
	require.NoError(t, err)

	assert.Equal(t, hostCSRLabelValue(testHostName), created.Labels[infrastructurev1beta1.HostCSRLabel])
	assert.True(t, strings.HasPrefix(created.Name, ByohCSRNamePrefix))
