...
const insecureWarning = `WARNING: --insecure disables TLS certificate verification for Platform9 API calls
         and is recorded in the host agent's kubeconfig, so it stays in effect for
         this host after onboarding completes. Use only on trusted networks.`
...
func init() {
	rootCmd.PersistentFlags().BoolVar(&insecure, "insecure", false,
		"Skip TLS certificate verification for Platform9 API calls. Also recorded in the "+
